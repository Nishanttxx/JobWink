...
/// Service wrapping Cerebras API via OpenAI-compatible REST endpoint (`https://api.cerebras.ai/v1/chat/completions`).
///
/// Provides fallback AI inference using Cerebras Llama 3.3 70B model.
class CerebrasService {
  static final CerebrasService instance = CerebrasService._internal();
  ...
  /// Initializes Cerebras service with API Key.
  void initialize(String apiKey) {
    if (apiKey.trim().isNotEmpty) {
      _apiKey = apiKey.trim();
      _isInitialized = true;
      debugPrint('[CerebrasService] Initialized successfully');
    } else {
      debugPrint('[CerebrasService] Warning: API key is empty');
    }
  }
  ...
  Future<ResumeData?> parseResume(Uint8List bytes, String mimeType) async {
    debugPrint('[CerebrasService] Processing resume...');
...
''';
    ...
    debugPrint('[DEBUG-PIPELINE-3] AI REQUEST SENT: provider=Cerebras, cleanTextLength=${cleanText.length}');

    try {
      final jsonMap = await generateJsonResponse(prompt);
      final jsonStr = jsonMap != null ? jsonEncode(jsonMap) : null;
      debugPrint('[DEBUG-PIPELINE-4] AI RAW RESPONSE: provider=Cerebras, length=${jsonStr?.length ?? 0}, snippet=${jsonStr != null && jsonStr.length > 200 ? jsonStr.substring(0, 200) : jsonStr}');
      if (jsonMap != null) {
        debugPrint('[DEBUG-PIPELINE-5] AI JSON: provider=Cerebras, keys=${jsonMap.keys.toList()}');
        final res = ResumeData.fromJson(jsonMap, rawText: cleanText);
        debugPrint('[DEBUG-PIPELINE-6] RESUME MODEL: provider=Cerebras, name="${res.fullName}", email="${res.email}", phone="${res.phone}", title="${res.title}", exp=${res.experience.length}');
        return res;
      }
    } catch (e) {
      debugPrint('[CerebrasService] parseResume error: $e');
    }
...
''';
    ...
    throw Exception('Cerebras failed to generate valid JSON for resume tailoring.');
  }
...
''';
    ...
    throw Exception('Cerebras failed to generate valid JSON for ATS analysis.');
  }
...
''';
    ...
    throw Exception('Cerebras failed to enhance summary.');
  }
  ...
  Future<Map<String, dynamic>?> generateJsonResponse(String prompt) async {
    final apiKey = _apiKey ?? AIConfig.cerebrasApiKey;
    if (apiKey.isEmpty) {
      debugPrint('[CerebrasService] Error: Cerebras API key is missing');
      return null;
    }

    final model = AIConfig.cerebrasModel;
    final baseUrl = AIConfig.cerebrasBaseUrl;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('[CerebrasService] REST API error status: ${response.statusCode}, body: ${response.body}');
      }
    } catch (err) {
      debugPrint('[CerebrasService] REST API call failed: $err');
    }
