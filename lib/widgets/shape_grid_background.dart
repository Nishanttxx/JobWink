...
/// with theme-adaptive borders, ambient animated movement, and optional hover trail highlights.
...
class _ShapeGridBackgroundState extends State<ShapeGridBackground>
  ...
  Duration _computeDuration() {
    final effectiveSpeed = widget.speed.clamp(0.05, 5.0);
    // Noticeable, smooth, fluid ambient drift: ~3.5 to 5.0s per cell cycle
    final ms = ((widget.squareSize / (effectiveSpeed * 28.0)) * 1000).round().clamp(1000, 15000);
  ...
  void initState() {
    ...
    widget.mousePositionNotifier?.addListener(_onExternalMousePosition);
  }

  @override
  void didUpdateWidget(covariant ShapeGridBackground oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.speed != widget.speed || oldWidget.squareSize != widget.squareSize) {
      _controller.duration = _computeDuration();
      if (!_controller.isAnimating) {
        _controller.repeat();
      }
    }
    if (oldWidget.mousePositionNotifier != widget.mousePositionNotifier) {
      oldWidget.mousePositionNotifier?.removeListener(_onExternalMousePosition);
      widget.mousePositionNotifier?.addListener(_onExternalMousePosition);
    }
  }
  ...
  @override
  void dispose() {
    widget.mousePositionNotifier?.removeListener(_onExternalMousePosition);
    _controller.removeListener(_updateCellOpacities);
    _controller.dispose();
    super.dispose();
  }

  void _onExternalMousePosition() {
    if (mounted) {
